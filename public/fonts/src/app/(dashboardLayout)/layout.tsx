    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-6 container mx-auto max-w-6xl">
          {data?.data?.user?.role === Roles.admin
            ? admin
            : data?.data?.user?.role === Roles.student
              ? student
              : data?.data?.user?.role === Roles.tutor
                ? tutor
                : data?.data?.user?.role === Roles.institute
                  ? institute
                  : data?.data?.user?.role === Roles.mentor
                    ? mentor
                    : data?.data?.user?.role === Roles.moderator
                      ? moderator
                      : null}
        </div>
      </SidebarInset>
